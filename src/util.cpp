...
const char * const BITCOIN_CONF_FILENAME = "simple.conf";
const char * const BITCOIN_PID_FILENAME = "simpled.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "simple" is a composite category enabling all SimpleCore-related debug output
            if(ptrCategory->count(string("simple"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SimpleCoreCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SimpleCoreCore
    // Mac: ~/Library/Application Support/SimpleCoreCore
    // Unix: ~/.simplecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SimpleCoreCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SimpleCoreCore";
#else
    // Unix
    return pathRet / ".simplecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty simple.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override simple.conf
