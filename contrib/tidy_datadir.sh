...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete SimpleCore database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no SimpleCore datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old SimpleCore datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected SimpleCore 0.7 datadir."
    ;;
  3)
    echo "Detected SimpleCore pre-0.8 datadir."
    ;;
  4)
    echo "Detected SimpleCore 0.8 datadir."
    ;;
esac
